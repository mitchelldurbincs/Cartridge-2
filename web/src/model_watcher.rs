...
//!
//! Watches for new ONNX model files and reloads them into the evaluator.
//! Also tracks model metadata (load time, file modification time) for display.
...
impl ModelWatcher {
    ...
    fn load_model(&self, path: &Path) -> Result<()> {
        ...
        let training_step = path.file_name().and_then(|n| n.to_str()).and_then(|name| {
            // Look for pattern like "model_step_000100" or just "step_100"
            name.split('_')
